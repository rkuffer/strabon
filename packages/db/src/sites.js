...
export async function querySites(params) {
    const sql = getSql();
    const { year, threshold, bboxMinLon, bboxMinLat, bboxMaxLon, bboxMaxLat } = params;
...
export async function upsertSite(site) {
    const sql = getSql();
    const location = site.lat != null && site.lon != null
 ...
 */
export async function searchSites(q, limit = 8) {
    const sql = getSql();
    const term = q.trim();
    if (term.length < 2)
        return [];
