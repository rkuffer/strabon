...
// ── Polities ──────────────────────────────────────────────────────────────────
export async function getAllPolities() {
    const sql = getSql();
...
export async function upsertPolity(polity) {
    const sql = getSql();
    await sql `
...
export async function upsertCulture(culture) {
    const sql = getSql();
    await sql `
...
function isReliableRefEntry(entry) {
    const v = entry?.value;
    return (!!v?.wikidata &&
        !!v?.name &&
        /^Q\d+$/.test(v.wikidata) &&
...
export async function syncReferentialsFromTimeline(timeline) {
    const sql = getSql();
    // ── Polities ──────────────────────────────────────────────────────────────
    const polityMap = new Map(); // wikidata_id → name
    for (const entry of timeline.polity?.entries ?? []) {
        if (!isReliableRefEntry(entry))
            continue;
        const v = entry.value;
    ...
    // ── Cultures ──────────────────────────────────────────────────────────────
    const cultureMap = new Map(); // wikidata_id → name
