...
function goldenHue(seed: string): number {
...
export function hullFillColor({ qid, familyQid }: HullColorInput): string {
  ...
  const lum =
    LIGHTNESSES[Math.floor(v / SATURATIONS.length) % LIGHTNESSES.length];
...
export function hullStrokeColor(input: HullColorInput): string {
