...
export function isInOccupationGap(
  ...
  let active: TrackEntry<SiteType> | null = null;
  for (const e of sorted) {
    if (e.from <= year) active = e;
    else break;
  }
...
export function computeDissolutionFromTimeline(
  ...
  if (TERMINAL_STATES.includes(last.value as string)) {
    return last.from;
  }
...
export function siteVisible(
  ...
  if (tl) {
    ...
    for (const track of [
      ...
      if (track?.entries) allFroms.push(...track.entries.map((e) => e.from));
    }
    if (tl.events) allFroms.push(...tl.events.map((e) => e.year));

    const firstFrom = allFroms.length ? Math.min(...allFroms) : null;
    if (firstFrom !== null && year < firstFrom) return false;
