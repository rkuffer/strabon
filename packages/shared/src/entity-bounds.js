...
function qidOf(e) {
    const q = typeof e?.value?.wikidata === "string" ? e.value.wikidata.trim() : "";
    return q || null;
}
/**
 ...
 */
function implicitEnd(entries, i, track) {
    const e = entries[i];
    if (e.to != null)
        return e.to;
 ...
 */
export function applyEntityBounds(timeline, bounds) {
    const tl = timeline;
    const conflicts = [];
    for (const track of BOUNDED_TRACKS) {
        const entries = tl[track]?.entries;
        if (!entries?.length)
            continue;
    ...
    return { timeline: tl, conflicts };
}
//# sourceMappingURL=entity-bounds.js.map